...
class ConcatLstm(NeuralNet):
	...
	def buildModel(self):
		premise_input = Input(shape=(premise_train[0].shape), dtype='float32', name='premise')
		premise_encoded = LSTM(50)(premise_input)

		hyp_input = Input(shape=(hypothesis_train[0].shape), dtype='float32', name='hypothesis')
		hyp_encoded = LSTM(50)(hyp_input)

		inputs_encoded = merge([premise_encoded, hyp_encoded], mode='concat')

		x = Dense(64,activation='relu')(inputs_encoded)
